...
class FakeAdapter implements DbAdapter {
  constructor(
    ...
    private readonly failures: { countRowsThrows?: string[] } = {},
  ) {}
  ...
  async fetchOrderedRows(args: {
    ...
    const rows = orderRows(table.rows, args.orderBy).slice(0, args.limits.maxRows);
...
describe('data-parity comparator', () => {
  ...
  test('tables above fullScanMaxRows compare a sample and report depth sampled', async () => {
    const columns = [ID_COL];
