/**
 * WADL parser -- COPIED VERBATIM from the discovery-service canonical source:
 *   discovery-service/src/services/findings/packFindingScanners/restWadl/wadlParser.ts
 *
 * Spec: 2026-06-03 OAS-YAML + WADL/XSD Contract Support for the API Behaviour
 * capture harness, Task Group 2.
 *
 * Why a copy (not a cross-import): the two services build as independent
 * TypeScript projects with separate `rootDir`s -- importing across the service
 * boundary is not possible without a shared package. The parser is PURE (its
 * only runtime dependency is `fast-xml-parser`, zero discovery coupling), so a
 * verbatim copy is the safe and idiomatic move. Keep this file BYTE-FOR-BYTE in
 * sync with the canonical source above; do not fork its behaviour here.
 *
 * ---------------------------------------------------------------------------
 * Original module docblock follows.
 * ---------------------------------------------------------------------------
 *
...
export interface ParseWadlOptions {
  ...
  /**
   * Identifier for the source file (e.g. `src/main/resources/app.wadl`) --
