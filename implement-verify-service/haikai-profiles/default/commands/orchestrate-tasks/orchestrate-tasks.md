...
In this spec's folder, create this file: `haikai/specs/[this-spec]/orchestration.yml`.
...
Loop through each task group in `haikai/specs/[this-spec]/tasks.md` and delegate its implementation to the assigned subagent specified in `orchestration.yml`.

For each delegation, provide the subagent with:
- The task group (including the parent task and all sub-tasks)
- The spec file: `haikai/specs/[this-spec]/spec.md`
- Instruct subagent to:
  - Perform their implementation
  - Check off the task and sub-task(s) in `haikai/specs/[this-spec]/tasks.md`
{{UNLESS standards_as_claude_code_skills}}
...
### NEXT: Verify each task group

After a task group's implementer has committed (with the D1 trailers `orchestrate-id` / `task-group-id` / `repo`) and triggered CI, verify that group before its dependents proceed. For each task group, dispatch the **verification-loop** subagent — or run `/verify-task-group` — passing `(orchestrate_id, task_group_id)`.

The verification-loop drives the group's `(repo × verifier)` cells, evaluates the D5 AND gate, and on a passing gate calls `advance()`, which releases the groups whose `depends_on` is now satisfied. A failing cell routes to `repair-engine`; the group (and its dependents) parks until its gate passes or a human resolves an escalation. CI/observe verdicts that land later re-enter the loop via the inbound-gateway, so a group may advance after the synchronous fan-out returns.

Do NOT advance a dependent group until its parent group's gate has passed — `advance()` releasing dependents is the only path that unblocks the DAG.
{{ENDIF use_claude_code_subagents}}
...
Follow these steps to generate this spec's ordered series of prompts texts, each in its own .md file located in `haikai/specs/[this-spec]/implementation/prompts/`.

LOOP through EACH task group in `haikai/specs/[this-spec]/tasks.md` and for each, use the following workflow to generate a markdown file with prompt text for each task group:

#### Step 1. Create the prompt markdown file

Create the prompt markdown file using this naming convention:
`haikai/specs/[this-spec]/implementation/prompts/[task-group-number]-[task-group-title].md`.
...
Read @haikai/specs/[this-spec]/spec.md to understand the context for this spec and where the current task fits into it.

Also read these further context and reference:
- @haikai/specs/[this-spec/]/planning/requirements.md
- @haikai/specs/[this-spec/]/planning/visuals
...
## Verify this task group

After implementing and committing this task group, verify it before moving on: run `/verify-task-group` for this `(orchestrate_id, task_group_id)`. It drives the group's `(repo × verifier)` cells, evaluates the D5 AND gate, and routes failures to `repair-engine`. Do NOT start the next group's prompt until this group's gate has passed.
```
...
Input those prompts into this chat one-by-one or queue them to run in order. Each prompt implements AND verifies its group (via `/verify-task-group`); only move to the next group's prompt once its gate has passed.

Progress will be tracked in `haikai/specs/[this-spec]/tasks.md`
```
