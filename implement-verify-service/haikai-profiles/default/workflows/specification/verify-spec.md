...
Read `haikai/specs/[this-spec]/planning/requirements.md` and verify:
...
ls -la [spec-path]/planning/visuals/ | grep -v "^total" | grep -v "^d"
```
...
1. **Read each visual file** in `haikai/specs/[this-spec]/planning/visuals/`
...
3. **Verify these design elements appear in**:
   - `haikai/specs/[this-spec]/spec.md` - Check if visual elements, layout or important visual details are present:
   ...
   - `haikai/specs/[this-spec]/tasks.md` - Confirm at least some tasks specifically reference:
     * Visual file names
     * Components shown in visuals
     * Layouts depicted in mockups

#### Check 4: Requirements Deep Dive
Read `haikai/specs/[this-spec]/planning/requirements.md` and create a mental list of:
...
Read `haikai/specs/[this-spec]/spec.md` and verify each section:
...
Read `haikai/specs/[this-spec]/tasks.md` and check each task group's tasks:
...
Create `haikai/specs/[this-spec]/verification/spec-verification.md` with the following structure:
...
See haikai/specs/[this-spec]/verification/spec-verification.md for full details.
```
