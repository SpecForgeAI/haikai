...
3. Compile the list of file references to those standards, one file reference per line, using this logic for determining which files to include:
   ...
   b. If the item under standards ends with "*" then it means that all files within this folder or sub-folder should be included. For example, `frontend/*` means include all files and sub-folders and their files located inside of `haikai/standards/frontend/`.
   c. If a file ends in `.md` then it means this is one specific file you must include in your list of files. For example `backend/api.md` means you must include the file located at `haikai/standards/backend/api.md`.
...
```
@haikai/standards/global/coding-style.md
@haikai/standards/global/conventions.md
@haikai/standards/global/tech-stack.md
@haikai/standards/backend/api/authentication.md
@haikai/standards/backend/api/endpoints.md
@haikai/standards/backend/api/responses.md
@haikai/standards/frontend/css.md
@haikai/standards/frontend/responsive.md
```
