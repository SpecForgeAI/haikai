#!/bin/bash
# Test script to verify Claude CLI can access Haikai profiles in local environment
...
echo "=================================================="
echo "Local Environment - Claude CLI Profile Access Test"
echo "=================================================="
echo ""
...
if [ -z "$HAIKAI_PROFILES_PATH" ]; then
    ...
    echo "  Setting it to $PROJECT_ROOT/haikai-profiles for this test..."
...
# Test 5: Check if Claude CLI is available
echo "Test 5: Testing Claude CLI availability..."
CLAUDE_CMD=""

# Check local node_modules first
if [ -f "$PROJECT_ROOT/node_modules/.bin/claude" ]; then
    CLAUDE_CMD="$PROJECT_ROOT/node_modules/.bin/claude"
    echo -e "${GREEN}✓ Claude CLI found in local node_modules${NC}"
    echo "  Location: $CLAUDE_CMD"
elif command -v claude &> /dev/null; then
    CLAUDE_CMD="claude"
    echo -e "${GREEN}✓ Claude CLI found in system PATH${NC}"
...
# Test Claude CLI
echo "  Running: $CLAUDE_CMD --help | head -20"
if "$CLAUDE_CMD" --help | head -20 > /dev/null 2>&1; then
...
# Test 6: Check if Claude can see the profiles
...
if [ -d "$PROJECT_ROOT/.venv-local" ]; then
    echo -e "${GREEN}✓ Virtual environment exists${NC}"
    echo "  Location: $PROJECT_ROOT/.venv-local"
    
    # Check if activated
    if [[ "$VIRTUAL_ENV" == "$PROJECT_ROOT/.venv-local" ]]; then
        echo -e "${GREEN}✓ Virtual environment is activated${NC}"
    else
        echo -e "${YELLOW}⚠ Virtual environment is not activated${NC}"
        echo "  Run: source .venv-local/bin/activate"
    fi
else
    echo -e "${RED}✗ Virtual environment not found${NC}"
    echo "  Run ./setup-local-env.sh to create it"
fi
echo ""

# Test 8: Create a minimal test project
echo "Test 8: Creating test project structure..."
TEST_DIR="$PROJECT_ROOT/api_workspace/test-project"
rm -rf "$TEST_DIR"
...
This is a test project to verify Claude CLI profile access in local environment.
EOF
...
# Test 9: Try a dry-run command (if possible)
echo "Test 9: Testing Claude CLI with --print flag..."
echo "  Running a simple echo test..."
cd "$TEST_DIR"
if echo "test" | "$CLAUDE_CMD" --print "Echo back: test" 2>&1 | grep -q "test\|Echo\|error" ; then
...
echo ""
echo "To run the API server with this environment:"
echo "  ./run-local.sh"
echo "=================================================="
