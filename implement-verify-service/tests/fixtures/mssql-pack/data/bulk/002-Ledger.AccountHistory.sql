-- Bulk load: Ledger.AccountHistory (table 2 of 4, FK-topological order)
-- Phase 2 of 5 — run AFTER the structural changesets, BEFORE foreign keys / indexes / reseed.
-- Identity columns (id): source values are PRESERVED. COPY writes identity columns directly; if you load via INSERT instead, use INSERT ... OVERRIDING SYSTEM VALUE to keep the source ids (the columns are GENERATED ALWAYS AS IDENTITY on the target).
...
COPY "Ledger"."AccountHistory" ("AccountID", "Balance", "ValidFrom", "ValidTo", "id") FROM STDIN WITH (FORMAT csv, NULL '\N');
