/**
 * `resolveBulkActionSet` -- the FRONTEND MIRROR of the gateway's shared PURE
 * cascade-set resolver (Spec 2, Task Group 5 of the discovery-review-unification
 * program F -> 0 -> 1 -> 2 -> 3).
 *
 ...
 *
 * CANONICAL HOME vs MIRROR:
 *   The CANONICAL implementation lives in the gateway at
 *   `gateway/src/services/discovery/resolveBulkActionSet.ts` (Spec 3's
 *   server-side coordinator imports it directly). The frontend grid cannot
 *   import gateway source across the build boundary, so THIS module is a
 *   byte-for-byte LOGIC MIRROR against the SAME snake_case `ReviewModel` wire
 *   shape. Parity between the two copies is held by the Group-4 wire-shape
 *   CONTRACT TEST + the Spec 2026-06-05-reject-cascade-correctness shared-fixture
 *   PARITY TEST -- a deliberate, declared mirror, exactly the idiom
 *   `frontend/src/api/__tests__/scopeRefType.contractWithGateway.test.ts`
 *   establishes for cross-package wire shapes (the frontend re-implements the
 *   logic; the contract test guards the shared shape). If you change the
 *   algorithm here, change the gateway copy identically (and vice versa).
 *
 * Given a SEED selection of candidate ids + the chosen review action and Spec 1's
 * deterministic `ReviewModel` (snake_case wire shape, ALREADY fetched on mount by
 * the grid via `getReviewModel` -- this function does NO fetch), it returns the
 * FULL touched set the cascade-aware bulk apply would act on:
 *
 ...
 *     dependency graph (Spec 1's blast radius is surface-only per candidate, so
 *     the resolver does the transitive walk). The reject-exclusivity walk is a
 *     monotone fixpoint over an only-growing reject set, also bounded by the same
 *     visited-set.
 */
 ...
 * below is BYTE-FOR-BYTE IDENTICAL to the gateway home (which binds this alias to
 * its own `EdgeKind`). This single alias line is the ONLY type-binding seam
 * between the two copies; every line of the algorithm that follows is identical
 * text.
 */
type ResolverEdgeKind = ReviewModelEdgeKind;
...
// ---------------------------------------------------------------------------
// Public input / output shapes (mirror the gateway helper's exported shapes)
// ---------------------------------------------------------------------------
...
export interface ResolveBulkActionSetInput {
  /**
   * The candidate ids the user seeded the action from (the grid's scope).
   * De-duplicated internally; order-insensitive.
   */
...
export interface ResolvedCandidate {
   ...
   */
  via_edge_kind: ReviewModelEdgeKind | null;
  /**
...
// ---------------------------------------------------------------------------
// Implementation (byte-for-byte mirror of the gateway helper's algorithm)
// ---------------------------------------------------------------------------
 ...
 *
 * Algorithm (deterministic, cycle-safe; identical to the gateway home):
