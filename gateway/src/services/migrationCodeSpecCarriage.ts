 ...
 *
 * SCL corpus stories are EXCLUDED explicitly (2026-09-03). They carry the
 * code-provenance tag too, and since the endpoint-identity join (2026-08-30)
 * their `apiEndpointIds` resolve, so they satisfied this predicate and — the
 * handler tests the code branch before the SCL branch — every external corpus
 * story was generated by the code carriage: the spec's Context repeated the
 * planner's "N behaviour-table row(s)" description while the body rendered
 * zero rows (the code carriage reads `business_logics.behavior`, not the
 * corpus), no acceptance criteria, and the scorer graded all of them F. A
 * corpus story is routed by `isSclCorpusStory` regardless of its endpoint ids.
 */
export function isCodeFactCarriageStory(story: CarriedStory): boolean {
  return (
    ...
    !(story.tags ?? []).includes(SCL_CORPUS_STORY_TAG) &&
