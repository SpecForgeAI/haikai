...
export interface IrColumn {
  ...
  findingIds: string[];
  // --- engine-feature facts (SQL Server pair programme, Spec 5.1) ---------
  /**
   * TRUE when the source computed column is PERSISTED (physically stored).
   * PostgreSQL's `GENERATED ALWAYS AS (…) STORED` reproduces a persisted
   * computed column exactly; a NON-persisted one is a PG 18 `VIRTUAL`
   * generated column. Null = the scan did not report persistence.
   */
  computedPersisted?: boolean | null;
  /**
   * Identity generation parameters from the source catalog. PostgreSQL
   * spells them `GENERATED BY DEFAULT AS IDENTITY (START WITH s INCREMENT
   * BY i)`; absent = the engine's defaults (1, 1).
   */
  identitySeed?: string | null;
  identityIncrement?: string | null;
  /**
   * `rowguidcol` / `sparse` storage attributes: no target equivalent, so
   * they become manifest notes rather than DDL.
   */
  isRowGuidCol?: boolean;
  isSparse?: boolean;
  /** Source-declared `filestream` storage — an item-5 decision, never DDL. */
  isFilestream?: boolean;
  /** Verbatim DB-level collation reported alongside a column collation. */
...
export interface IrIndex {
  ...
  /**
   * Non-key covering columns (SQL Server `INCLUDE (...)`; PostgreSQL has the
   * same clause since 11). Empty/absent = a plain key-only index.
   */
  includeColumns?: string[] | null;
  /**
   * TRUE when the source index is DISABLED. A disabled index is not
   * enforcing anything at source, so it emits with a loud note rather than
   * silently becoming an enforced target index.
   */
  isDisabled?: boolean;
}

/**
 * One source object with NO like-for-like target shape (SQL Server pair
 * programme, Spec 5.1/5.5). The `kind` vocabulary is open on purpose — it is
 * whatever the scan's `extendedObjects[]` / feature findings reported — and
 * the pack maps each kind to its decision category + emulation.
 */
export interface IrExtendedObject {
  kind: string;
  schemaName: string | null;
  objectName: string;
  /** Verbatim engine detail (base object, queue target, assembly name, …). */
  detail: Record<string, unknown> | null;
  /** Source body when one exists (CLR assembly binding, Broker contract, …). */
  sourceBody?: string | null;
  /** Named untranslatable reason carried by the scan (never invented here). */
  untranslatableReason?: string | null;
  findingIds: string[];
}

/** System-versioned (temporal) table facts merged from the scan findings. */
export interface IrTemporal {
  /** `system_versioned` (the current table) or `history` (its history half). */
  temporalType: string;
...
export interface IrTable {
  ...
  /**
   * `isDisabled` / `isNotTrusted` (Spec 5.3): a source CHECK the engine is
   * not enforcing, or one whose existing rows were never validated against
   * it (SQL Server `WITH NOCHECK`). Such a constraint emits `NOT VALID` —
   * enforced for new rows, never failing the apply on data the source itself
   * never checked. Absent/false = an ordinary enforced CHECK.
   */
  ...
  findingIds: string[];
  /**
   * System-versioned temporal facts (Spec 5.5). Present on BOTH halves of a
   * temporal pair: the current table carries `system_versioned`, its history
   * table carries `history` so it is never migrated as a standalone table.
   */
  temporal?: IrTemporal | null;
  /** Source table is MEMORY_OPTIMIZED (In-Memory OLTP) — a note, not DDL. */
  memoryOptimized?: boolean;
  /** Full-text indexes declared on this table (Spec 5.5 emulation input). */
  fullTextIndexes?: Array<{ name: string; catalog: string | null; columns: string[] }>;
}
...
export interface IrForeignKey {
  ...
  /**
   * The source FK is NOT TRUSTED (created / re-enabled WITH NOCHECK): the
   * existing rows were never validated against it, so the target constraint
   * emits `NOT VALID` (Spec 5.3). Absent/false = an ordinary trusted FK.
   */
  isNotTrusted?: boolean;
}
...
export interface IrSequence {
  ...
  findingIds: string[];
  // --- full generation detail (Spec 5.3: "sequences with full detail") ----
  /** Verbatim increment (string — bigint-safe). null = the engine default. */
  increment?: string | null;
  minValue?: string | null;
  maxValue?: string | null;
  cycle?: boolean;
  /** Verbatim source data type (`bigint`, `int`, `numeric(18,0)`, …). */
...
export interface SourceSchemaIr {
  ...
  /**
   * Objects with NO like-for-like target shape, merged from the scan's
   * feature findings (Spec 5.5). Each drives a decision category + the
   * emulation the pack builds. Optional so IR literals in tests stay valid.
   */
  extendedObjects?: IrExtendedObject[];
  /**
   * Source database default collation, verbatim (Spec 5.4). Drives the ONE
   * pack-wide `collation` decision when it implies case-insensitive
   * matching. Null = the scan did not report it.
   */
  databaseCollation?: string | null;
...
export type PackDecisionCategory =
  ...
  | 'delta_key'
  /**
   * SQL Server pair programme, Spec 5.5 (AMS `chk_dmpd_category` extended by
   * changeset 234). Every SQL-Server-only object shape with no like-for-like
   * PostgreSQL form is EITHER emulated by the pack with a confirmable
   * default OR gated here with named options — never manual residue, never a
   * silent drop. `cross_database_reference` and `indexed_view` are OUT by
   * owner ruling: their decisions carry the named untranslatable reason and
   * offer no "translate" option at all.
   */
  | 'temporal_table'
  | 'fulltext_index'
  | 'xml_method'
...
export interface PackManifest {
  ...
  requires_translation_spec_2: Array<{
    ...
    finding_ids: string[];
    /**
     * Named untranslatable reason (2026-09-11, Spec 5.5): the object rides
     * the queue so the workbench SHOWS it, but is never attempted.
     */
  ...
  /**
   * Index-shaped engine differences the emitter resolved or noted (a
   * columnstore index dropped to b-tree, a DISABLED source index emitted, a
   * typed index mapped to GIN/GiST). Absent on packs generated earlier.
   */
  index_notes?: string[];
  /**
  ...
  /**
   * Columns whose SOURCE collation is case-INSENSITIVE (Spec 5.4). The data
   * parity comparator enables the `collation-case` strategy on EXACTLY these
   * columns — never estate-wide, never guessed. Absent on packs generated
   * before the pair programme.
   */
  collation_affected_columns?: string[];
  /** The resolved pack-wide collation posture, or null while it is open. */
  collation_posture?: string | null;
  /**
   * Columns whose source temporal precision exceeds the target's microsecond
   * resolution (`datetime2(7)` / `time(7)` / `datetimeoffset(7)`): the loader
   * TRUNCATES the 7th fractional digit and the pair rule governs parity.
   */
  precision_loss_columns?: Array<{ column: string; source_type: string; note: string }>;
  /**
   * Target-side extensions the pack's chosen mappings REQUIRE (citext,
   * ltree, postgis, pg_cron). A loud runbook prerequisite, never an
   * optional extra.
   */
  target_extensions_required?: string[];
  /**
   * The item-5 emulations the pack BUILT (temporal history tables, full-text
   * tsvector columns, …) with the decision each honoured and the file that
   * carries the DDL. Visible accounting: an emulation is never invisible.
   */
  emulations?: Array<{
    kind: string;
    object_ref: string;
  ...
  /**
   * Objects the scan reported with no like-for-like target shape and the
   * disposition each received (emulated / decision open / named OUT).
   */
  extended_objects?: Array<{
    kind: string;
    object_ref: string;
