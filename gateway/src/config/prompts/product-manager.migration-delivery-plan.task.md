...
4. Classify an item `exceptional` when its model facts show anything non-uniform: attached discovery findings, live conflicts, a missing API behaviour baseline, complex SOAP message schemas, or readiness other than `ready_for_spec`. (The gateway also enforces these as hard overrides in code.)
