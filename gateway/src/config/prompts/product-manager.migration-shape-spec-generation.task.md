...
**The anti-rules above forbid CONTRADICTING or GAP-FILLING — they never forbid NAMING the captured stack.** When decisions ARE captured (in either context block or a `TARGET TECHNOLOGY STACK` block of the user prompt), the spec MUST name the concrete captured technologies and their framework-level components in its implementation steps, acceptance criteria and tests. A technology-neutral spec ("register the handler with the runtime's configuration mechanism") against a captured stack is a FAILURE to follow this context, exactly as a contradiction would be. For planner-authored foundation and scaffold stories in particular: the target repository starts EMPTY and the application is created by the FIRST-sequenced scaffold story from the captured stack — "follow the target codebase's existing conventions" can never mean silence about technology, because those conventions are established BY these stories from the captured decisions.
