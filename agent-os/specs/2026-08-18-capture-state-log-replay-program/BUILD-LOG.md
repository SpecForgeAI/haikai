...
| 9b | S0 outcome VISIBLE on the DB run detail (2026-08-19 shakedown gap: outcome was recorded but never rendered) | feature/s0-snapshot-visible-outcome | built+merged ee27748d (DiscoveryRunDetailPage derives steps_payload.database.s0Snapshot -> dedicated "S0 snapshot" row: green "Taken with this scan" + table count / red FAILED + recorded reason / neutral Skipped + reason; absent field [code runs, pre-feature runs] renders nothing; 3 vitest; NOTE the auto-S0 POST needs AMVS (and the sidecar for a Sybase source) RUNNING at scan completion — a failed pin is recoverable by re-running the DB scan while the DB is still untouched) |
| 3e | ONE SCAN, ONE REVIEW, ONE SAVE (2026-08-20, main 968d8839, user's 4th one-action ruling): the CODE SCAN itself emits endpoint_data_effects CANDIDATES via new step 9.7 — runStructuralScanStep returns {outcome, corpus}; deterministic corpus derivation (0.9, 'scl_corpus') for write endpoints discovery's mining left bare + vocabulary-guarded LLM proposals (0.65, 'llm_proposal') via the EXISTING gateway gap-fill relay (empty vocabulary = loud skip: save the DB scan first); candidates bulk-saved into the run, reviewed + saved like everything else (save-back resolves names→ids, skips honestly); summary rides steps_payload...effectCandidates; 3d's button = RECOVERY path for pre-existing scans | feature/one-scan-effect-candidates | built+merged 968d8839 |
| 3d | EFFECT-MAP BACKFILL (2026-08-20, main c3a31c70, user ruling after the live preflight listed 34 unmapped write endpoints): (1) deterministic corpus derivation — unmapped mutating endpoints matched to SCL http-rooted behaviour tables (annotation-pass helpers reused; longest-fragment discipline), call graph walked to boundary contracts, verbatim SQL parsed for write targets, filtered vs committed physical entities, APPLIED immediately; (2) guarded LLM proposals for the rest (8/batch, closed table vocabulary = hallucination guard, review modal, approved subset applied); NEW MCP tool apply_endpoint_effects (additive endpoint_data_effects append, dep_phy_ convention, never duplicates, tagged effect_map_backfill); gateway routes effect-map-backfill/{run,apply}; "Backfill effect maps" button on API Behaviour Baselines page; preflight remedy line points at it; after apply just Start again (preflight re-reads live) | feature/effect-map-backfill | built+merged c3a31c70 |
| 3c | Journey-audit round 2 (2026-08-20, main 6664a792, user: "no gaps"): capture-session Diagnostics section (refusals/residue/S0 advisories grouped, halt red/warning amber, 5 msgs+count, re-read per poll) + S0 RESTORE FROM THE UI (failed session naming S0 → S0RestorePanel: latest-manifest summary via new gateway s0-snapshot/latest proxy, credential form prefilled from REDACTED db config [password re-entered; body-only credentials], consent checkbox + service-side confirm gate, restore report incl. fingerprint verification via new s0-snapshot/restore proxy); sibling SCL fixes in the SCL BUILD-LOG (annotation-pending tab line, planner-provenance header line) | feature/ux-gaps-round-2 | built+merged 6664a792 |
| 3b | Compensation preflight WIRED to the wizard (2026-08-19 journey-audit gap: spec 3's GET route had zero FE callers — captures started blind) | feature/ux-gaps-annotation-preflight | built+merged e70b0627 (wizard Start click fetches GET .../compensation-preflight FIRST — before any Postman/log pre-fire and before the configured PATCH — and gates on window.confirm: unmapped write-endpoint list [10 shown, +N more] + save-back remedy / compensation-INACTIVE warning when the model does not resolve / loud prompt on a preflight outage [operator decides, never silent]; 409-override re-submit never re-prompts; new gateway GET proxy beside the 12 POST action proxies; pure builders in compensationPreflightSupport; 6+4 vitest + 1 gateway jest; 3 existing wizard suites armed w/ clean preflight) |
