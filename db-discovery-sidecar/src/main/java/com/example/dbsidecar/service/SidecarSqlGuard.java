 ...
 *       EXECUTE, CALL, GRANT, REVOKE, CREATE, OPENROWSET, OPENQUERY, BULK,
 *       SP_*}.</li>
...
public final class SidecarSqlGuard {
    ...
    private static final List<String> FORBIDDEN_KEYWORDS = List.of(
            ...
            "TRUNCATE",
            "EXEC",
            // EXECUTE is NOT caught by the EXEC pattern: \bEXEC\b does not
            // match inside "EXECUTE" (no word boundary between C and U), so
            // the long spelling needs its own entry (SPEC-1 §1.5).
            "EXECUTE",
            "CALL",
            "GRANT",
            "REVOKE",
            "CREATE",
            // SQL Server ad-hoc data access + bulk import: each reads or
            // writes OUTSIDE the connected database (a file, a linked server,
            // a remote provider) and none belongs on a read-only
            // introspection surface. Blocked for BOTH engines (SPEC-1 §1.5).
            "OPENROWSET",
            "OPENQUERY",
            "BULK"
    );
