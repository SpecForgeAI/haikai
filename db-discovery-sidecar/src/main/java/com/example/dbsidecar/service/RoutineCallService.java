 ...
 *
 * <p>Sibling of {@link DbQueryService} (read-only SELECTs) and
 * {@link DbMutationService} (the compensation write batch). Like the
 ...
 * password-masked (via {@link DbMutationService#maskPassword}) before
 * leaving the JVM.</p>
 */
@Service
public class RoutineCallService {

    private static final Logger LOG = LoggerFactory.getLogger(RoutineCallService.class);
    ...
    public CallResponse call(
        ...
        try {
            assertNoTableValuedParams(ordered);
        } catch (final IllegalArgumentException e) {
            LOG.warn("[diag-sidecar] op=call status=rejected reason={}", REASON_TVP_UNSUPPORTED);
            return CallResponse.failure(e.getMessage(), null);
        }
        ...
        String driverUsed = this.connections.lastAttemptedDriverName(options);
        Integer returnStatusValue = null;
        CallResponse.ErrorDetail errorDetail = null;
        long timingMs = 0L;

        try {
            final ConnectionFactory.Opened opened = this.connections.open(options, false);
            conn = opened.connection();
            driverUsed = opened.driverName();
            conn.setAutoCommit(true);
            applySessionSets(conn, sessionSets, engine);
            ...
            try (CallableStatement cs = conn.prepareCall(callString)) {
                ...
                if (returnStatus) {
                    ...
                    registerFunctionResult(cs, functionResultDescriptor, engine);
                }
                bindAll(cs, ordered, indexOffset, engine);
                ...
                try {
                    capped = walkResults(
                            ...
                            maxRowsPerResultSet, maxResultSets, password, engine);
                ...
                drainWarnings(safeWarnings(cs), messages, password, engine);
                drainWarnings(safeWarnings(conn), messages, password, engine);
            ...
            return CallResponse.failure(
                    DbMutationService.maskPassword(e.getMessage(), password), driverUsed);
    ...
    enum BindFamily {
        ...
        STRING,
        /** SQL Server nchar / nvarchar / ntext / xml -- bound as NATIONAL text. */
        NSTRING,
        /** SQL Server datetimeoffset -- bound through its own driver type. */
        DATETIMEOFFSET
    }

    /** Named refusal reason for a table-valued parameter on {@code /call}. */
    static final String REASON_TVP_UNSUPPORTED = "tvp_unsupported_in_call";

    /**
     * {@code microsoft.sql.Types.DATETIMEOFFSET}. Spelled as the constant it
     * is (-155) with the driver class named in the comment rather than
     * imported, so the binder table stays readable next to the {@link Types}
     * entries around it.
     */
    static final int MSSQL_DATETIMEOFFSET = microsoft.sql.Types.DATETIMEOFFSET;

    /**
     * Refuse a table-valued parameter LOUDLY and by NAME. A TVP has no
     * positional {@link CallableStatement} binding, and silently binding its
     * string form would capture behaviour the routine never actually saw.
     */
    static void assertNoTableValuedParams(final List<CallParam> params) {
        if (params == null) {
            return;
        }
        for (final CallParam p : params) {
            if (p != null && p.isTableValued()) {
                throw new IllegalArgumentException(
                        "Parameter '" + (p.getName() == null ? "?" : p.getName())
                                + "' is a table-valued parameter; /call cannot bind one ("
                                + REASON_TVP_UNSUPPORTED + ").");
            }
        }
    }
    ...
    static BindFamily familyFor(final String sourceType) {
        return familyFor(sourceType, SidecarEngine.SYBASE);
    }

    /**
     * Which setter family binds this type on this engine (SPEC-1 §1.5).
     *
     * <p>The two engines' type vocabularies overlap but do NOT coincide, and
     * two of the collisions are traps: {@code timestamp} is a ROWVERSION on
     * both (binary, never a datetime), while {@code datetime2} /
     * {@code datetimeoffset} / {@code uniqueidentifier} / {@code sql_variant}
     * exist only on SQL Server. Keeping one table per engine is what stops a
     * SQL Server {@code nvarchar} being bound through a narrowing
     * {@code setString} or a {@code datetimeoffset} losing its zone.</p>
     */
    static BindFamily familyFor(final String sourceType, final SidecarEngine engine) {
        if (engine == SidecarEngine.MSSQL) {
            return mssqlFamilyFor(sourceType);
        }
        switch (baseTypeToken(sourceType)) {
            case "int":
            case "integer":
            case "smallint":
            case "tinyint":
    ...
    /** The SQL Server bind-family table (SPEC-1 §1.5). Pure. */
    static BindFamily mssqlFamilyFor(final String sourceType) {
        switch (baseTypeToken(sourceType)) {
            case "int":
            case "integer":
            case "smallint":
            ...
            case "datetime2":
                return BindFamily.TIMESTAMP;
            case "datetimeoffset":
                return BindFamily.DATETIMEOFFSET;
            case "date":
                return BindFamily.DATE;
            case "time":
                return BindFamily.TIME;
            ...
            case "varbinary":
            case "image":
            // SQL Server's "timestamp" is a rowversion (binary(8)) here too.
            case "timestamp":
            case "rowversion":
                return BindFamily.BYTES;
            case "nchar":
            ...
            case "sysname":
            case "xml":
                return BindFamily.NSTRING;
            // uniqueidentifier binds as its canonical 36-character string;
            // sql_variant, hierarchyid and the spatial types bind as the
            // driver's own string form (ToString() / WKT).
            default:
                return BindFamily.STRING;
        }
    }
     ...
     */
    static int jdbcTypeFor(final String sourceType) {
        return jdbcTypeFor(sourceType, SidecarEngine.SYBASE);
    }

    /** The {@link Types} constant for this type on this engine (SPEC-1 §1.5). */
    static int jdbcTypeFor(final String sourceType, final SidecarEngine engine) {
        if (engine == SidecarEngine.MSSQL) {
            return mssqlJdbcTypeFor(sourceType);
        }
        switch (baseTypeToken(sourceType)) {
            case "int":
            case "integer":
    ...
    /** The SQL Server typed-NULL / OUT-registration table (SPEC-1 §1.5). Pure. */
    static int mssqlJdbcTypeFor(final String sourceType) {
        switch (baseTypeToken(sourceType)) {
            case "int":
            case "integer":
                return Types.INTEGER;
            ...
            case "char":
                return Types.CHAR;
            // A uniqueidentifier is a fixed 36-character canonical string:
            // CHAR is the shape mssql-jdbc accepts for both bind and OUT.
     ...
     * encoder in {@code DbQueryService.normalizeWireValue}, so a value
    ...
    private static void bindAll(
        ...
        for (int i = 0; i < ordered.size(); i++) {
            ...
            if (p.isOutput()) {
                cs.registerOutParameter(index, jdbcTypeFor(p.getSourceType(), engine));
            }
            if (p.isInput()) {
                bindParam(cs, index, p, engine);
            }
        }
    }
    ...
    static void bindParam(final CallableStatement cs, final int index, final CallParam p)
            throws SQLException {
        bindParam(cs, index, p, SidecarEngine.SYBASE);
    }

    /** Engine-aware bind (SPEC-1 §1.5). */
    static void bindParam(
        ...
        final int jdbcType = jdbcTypeFor(p.getSourceType(), engine);
        if (p.bindsNull()) {
        ...
        try {
            switch (familyFor(p.getSourceType(), engine)) {
                case INTEGRAL:
                ...
                case BYTES:
                    ...
                    break;
                case NSTRING:
                    cs.setNString(index, value);
                    break;
                case DATETIMEOFFSET:
                    // microsoft.sql.DateTimeOffset carries the zone the wire
                    // string declares; a plain setTimestamp would drop it.
                    cs.setObject(index, parseWireDateTimeOffset(value), MSSQL_DATETIMEOFFSET);
                    break;
                case STRING:
                default:
    ...
    /**
     * Wire string to {@code microsoft.sql.DateTimeOffset}. Accepts the ISO
     * form the sidecar emits ({@code yyyy-MM-ddTHH:mm:ss.fffffff+HH:MM}) and,
     * leniently, a zone-less value -- which is read as UTC, the sidecar's own
     * pinned zone, rather than silently taking the JVM default.
     */
    static microsoft.sql.DateTimeOffset parseWireDateTimeOffset(final String value) {
        final String trimmed = value == null ? "" : value.trim();
        try {
            return microsoft.sql.DateTimeOffset.valueOf(
                    java.time.OffsetDateTime.parse(trimmed.replace(' ', 'T')));
        } catch (final java.time.format.DateTimeParseException e) {
            final LocalDateTime naive = parseWireDateTime(trimmed);
            return microsoft.sql.DateTimeOffset.valueOf(
                    java.sql.Timestamp.valueOf(naive), 0);
        }
    }
    ...
    private static void registerFunctionResult(
        ...
        if (descriptor != null && descriptor.getSourceType() != null) {
            cs.registerOutParameter(1, jdbcTypeFor(descriptor.getSourceType(), engine));
            return;
        }
        try {
                ...
                if (rs != null) {
                    if (resultSets.size() < maxResultSets) {
                        resultSets.add(collectResultSet(
                                rs, resultSets.size() + 1, maxRowsPerResultSet, engine));
                    } else {
    ...
    private static CallResponse.ResultSetPayload collectResultSet(
        ...
        while (rs.next()) {
            ...
            for (int i = 1; i <= columnCount; i++) {
                row.add(DbQueryService.readWireValue(rs, i, md, engine));
            }
            rows.add(row);
        }
    ...
    private static Integer readReturnStatus(
        ...
        try {
            ...
            if (function) {
                ...
                outputParams.put(key, DbQueryService.normalizeWireValue(raw));
            ...
            messages.add(new CallResponse.Message(
                    ...
                    "return placeholder unreadable: "
                            + DbMutationService.maskPassword(e.getMessage(), password)));
        }
        return null;
    }
    ...
    private static void readOutputParams(
        ...
        for (int i = 0; i < ordered.size(); i++) {
            ...
            try {
                outputParams.put(key, DbQueryService.normalizeWireValue(cs.getObject(index)));
            } catch (final SQLException e) {
    ...
    static String classifyMessageKind(final int number, final Integer severity) {
        return classifyMessageKind(number, severity, SidecarEngine.SYBASE);
    }

    /**
     * Engine-aware classification (SPEC-1 §1.5). SQL Server's {@code THROW}
     * raises with a USER error number (>= 50000) at severity 16; when the
     * driver surfaces it as a warning rather than an exception the severity
     * can be absent, so the number alone is enough to classify it as a
     * {@code raiserror}. The wire vocabulary is unchanged --
     * {@code print | raiserror | info} -- because a consumer comparing two
     * engines' behaviour must see one set of kinds, not two (wire contract
     * §5).
     */
    static String classifyMessageKind(
            final int number, final Integer severity, final SidecarEngine engine) {
        if (number == 0) {
            return "print";
        }
        ...
        if (engine == SidecarEngine.MSSQL && number >= 50000) {
            return "raiserror";
        }
        return "info";
    }
    ...
    private static void drainWarnings(
        ...
        while (warning != null && guard++ < MAX_WARNING_CHAIN) {
            final Integer severity = severityOf(warning);
            final Integer state = stateOf(warning);
            final int number = warning.getErrorCode();
            messages.add(new CallResponse.Message(
                    classifyMessageKind(number, severity, engine),
                    number,
                    severity,
                    state,
                    DbMutationService.maskPassword(warning.getMessage(), password)));
            SQLWarning next;
            try {
    ...
    /**
     * Severity of a driver exception / warning.
     *
     * <p>SQL Server exposes it as a FIRST-CLASS field --
     * {@code SQLServerException.getSQLServerError().getErrorSeverity()} -- so
     * mssql-jdbc is read directly. jConnect only exposes it through
     * {@code EedInfo}, which is read reflectively because the jConnect jar is
     * optional and must not be a compile dependency. jTDS exposes neither, and
     * null is the honest answer there.</p>
     */
    static Integer severityOf(final Object thrown) {
        if (thrown instanceof com.microsoft.sqlserver.jdbc.SQLServerException) {
            final com.microsoft.sqlserver.jdbc.SQLServerError error =
                    ((com.microsoft.sqlserver.jdbc.SQLServerException) thrown)
                            .getSQLServerError();
            if (error != null) {
                return error.getErrorSeverity();
            }
        }
        return reflectInt(thrown, "getSeverity");
    }

    /** State of a driver exception / warning; see {@link #severityOf}. */
    static Integer stateOf(final Object thrown) {
        if (thrown instanceof com.microsoft.sqlserver.jdbc.SQLServerException) {
            final com.microsoft.sqlserver.jdbc.SQLServerError error =
                    ((com.microsoft.sqlserver.jdbc.SQLServerException) thrown)
                            .getSQLServerError();
            if (error != null) {
                return (int) error.getErrorState();
            }
        }
        return reflectInt(thrown, "getState");
    }
     ...
     */
    private static void applySessionSets(
        ...
        try (Statement st = conn.createStatement()) {
            for (final String line : sessionSets) {
                st.execute(CallSqlGuard.assertSessionSetLine(line, engine));
            }
        }
    }
