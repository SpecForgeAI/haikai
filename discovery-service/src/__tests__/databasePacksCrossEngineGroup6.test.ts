 ...
 *
 *   (a) Cross-engine candidate shape consistency: Postgres and Sybase packs
 *       MUST emit the same {@code DatabaseCandidatePayload} shape for an
 *       equivalent introspection input. Only {@code data.dbEngine} differs.
 *
...
function makeConfig(engine: 'postgres' | 'sybase'): DatabaseDiscoveryConfig {
  return {
    dbEngine: engine,
    host: 'db.test',
...
function makeCtx(engine: 'postgres' | 'sybase'): DatabaseDiscoveryPackContext {
  return {
...
describe('Cross-engine DatabaseCandidatePayload shape (Spec 2026-05-16, Group 6 gap-fill)', () => {
  it('Postgres and Sybase emitCandidates produce identical shapes for the same introspection input (only data.dbEngine differs)', async () => {
    const intro = sharedIntrospection();
    const emptyProfile: ProfileResult = { tables: [], skippedTables: [] };

    // emitCandidates is a pure introspection->payload transformer on both
  ...
  it('Both packs expose the same DatabaseDiscoveryPack method surface (interface conformance)', () => {
    const pgPack = new PostgresDiscoveryPack();
    const sybPack = new SybaseDiscoveryPack();
